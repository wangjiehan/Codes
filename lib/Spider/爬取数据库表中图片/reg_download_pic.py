...
if __name__ == "__main__":
	...
	# sql = """SELECT content FROM `aiw_read_article` WHERE `index` < 250"""
	# sql = """SELECT content FROM `aiw_read_article` WHERE `index` = 71129"""
	try:
		cursor.execute(sql)
		...
		for i, element in enumerate(results):
			...
			if html:
				...
				load_image(html, download_path, pic_name)
				print("Success in saving the picture where `index` = %d" % (i + 1))
